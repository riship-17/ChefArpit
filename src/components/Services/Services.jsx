...
const Services = () => {
  ...
  useGSAP(() => {
    ...
    mm.add({
      ...
      if (isDesktop) {
        gsap.from(cards, {
          y: 20, opacity: 0, duration: 0.4, stagger: 0.05, ease: EASE,
          ...
          scrollTrigger: { trigger: gridRef.current, start: 'top bottom', once: true, invalidateOnRefresh: true }
        });
      } else {
        cards.forEach(card => {
          gsap.from(card, {
            y: 20, opacity: 0, duration: 0.35, ease: EASE,
      ...
      gsap.from(premiumRef.current, {
        ...
        scrollTrigger: { trigger: premiumRef.current, start: 'top bottom', once: true, invalidateOnRefresh: true }
      });
    });
  }, { scope: sectionRef });
